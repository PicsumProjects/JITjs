...
function checkInstr(instr) {
    for (const arg of instr.args)
        if ((((arg >>> 0) > 64) || ((arg >>> 0) !== arg)) && (!((instr.type > 75) && (instr.type < 84))))
            throw new Error('Invalid instruction');
    if ((instr.type >>> 0) > LASTINSTR)
        throw new Error('Invalid instruction');
}
;
export function asmModule(mod, name) {
    var i = 0;
...
;
function asmFunc(instrs, count) {
    let base = `    function $${Number(count)}()\n    {\n`;
    for (const instr of instrs.instrs)
        base += '        ' + asmSingleInstr(instr);
    return base + '    };\n\n';
}
;
function asmSingleInstr(instr) {
    if (!("asm" in Object.keys(backendList)))
